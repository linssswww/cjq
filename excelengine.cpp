...
bool ExcelEngine::Open(UINT nSheet, bool visible)
{
    ...
    /*如果指向的文件不存在，则需要新建一个*/
	//sXlsFile = "D:\\workfile\\CGD\\Trainner\\debug\\count.xls";
    QFile f(sXlsFile);
    if (!f.exists())
    {
        bIsANewFile = true;
...
int ExcelEngine::findLable(ExcelEngine & ee)
{
	int row = 0;
...
bool ExcelEngine::ReadDataToVector(vector<vector<QString>> &table)
{
	int rowcnt = nStartRow + nRowCount;
	int columncnt = nStartColumn + nColumnCount;
	...
	for (int i = nStartRow + 1, r = 0; i < rowcnt; i++, r++)  //行
	{
		...
		for (int j = nStartColumn, c = 0; j < columncnt; j++, c++)  //列
		{
			...
			{
				QString str = cell->dynamicCall("Value2()").toString();
				vec_row.push_back(str);
			}
		}
		table.push_back(vec_row);
	}
